#' BHMOI for clinical trials with subgroups and binary outcomes (CTSB)
#'
#'
#'
#' @param J Number of responses in each subgroup.
#' @param W Number of patients in each subgroup.
...
#' @param init.thetas A list includes the distributions of interested parameters which are the non-informative posterirors.
...
#' res = CTSB(J=J, W=W, nGroups=nGroups, a=0.3)
#' res$OBIs
...
#'
#' @export
CTSB<-function(J, W, nGroups, a=0.3, weighted=FALSE, b=5, alpha.min=1, alpha.max=50, beta=10, init.thetas=NULL,
                    tau0=0.01, mu0=.logit(0.1), K=0, fast=1, n_sim=3, seed=123, nIter=2000,kmIters=50, grid=1000){
  # Step 1, run initApproxDists()
  # To insure result reproducible, we first need to make init.thetas reproducible. Yes, confirmed!
  if(is.null(init.thetas)){
    init.thetas <- initTranPostBinary(J = J, W = W, nGroups = nGroups, alpha=alpha.min/100,
                                      beta=beta, tau0=tau0, mu0=mu0, seed=seed, nIter=nIter)
  }
  # Step 2, get best clustering result by maximizing OCI
  # To insure result reproducible, second, we have to make sure the calculation of OVL reproducible.
  # Third, we also need to make sure the K-Means algorithm reproducible.
  oci.res <- clusterMaxOCI(dataSets=init.thetas, data_type = 1, a=a, weighted=weighted, b=b,
  ...
  for (j in 1:length(clusters_ids)) {
    ...
    if(K_m==1){ # cluster only has one distribution, no information borrowing.
      thetas_m = matrix(init.thetas[[(clusters_ids[[j]])]],ncol = 1)
    }else{ # cluster has more than one distributions
      J_m = J[clusters_ids[[j]]]
      W_m = W[clusters_ids[[j]]]
      ...
      mydata = list(Y=J_m, n=W_m, K=K_m, a=als[j], b=beta, mu0 = c_mu, tau0 = tau0)
      if(is.null(seed)){
        thetas_m = rstan::extract(rstan::sampling(stanmodels$tran_bhm_general,mydata, iter = nIter,
                                                  chains = 1, show_messages=F,refresh=F))$p_k
      }else{
        thetas_m = rstan::extract(rstan::sampling(stanmodels$tran_bhm_general,mydata,seed=seed,
                                                  iter = nIter, chains = 1, show_messages=F,refresh=F))$p_k
      }
    }
  ...
  class(resL)<-"pmdfs"
  return(list(initPost=init.thetas, borrowedPost=resL, maxOCIres=oci.res, OBIs=obis))
}
